...
void DrawPlayer(void)
{
	...
	// ポリゴンの描画
	pDevice->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, NUM_PLAYER, g_vertexWkPlayer, sizeof(VERTEX_2D));
}
...
HRESULT MakeVertexPlayer(void)
{
	...
	// テクスチャ座標の設定 (*33)
	g_vertexWkPlayer[0].tex = D3DXVECTOR2( 0.0f, 0.0f );
	g_vertexWkPlayer[1].tex = D3DXVECTOR2( 1.0f/TEXTURE_PATTERN_DIVIDE_X, 0.0f );
	g_vertexWkPlayer[2].tex = D3DXVECTOR2( 0.0f, 1.0f/TEXTURE_PATTERN_DIVIDE_Y );
	g_vertexWkPlayer[3].tex = D3DXVECTOR2( 1.0f/TEXTURE_PATTERN_DIVIDE_X, 1.0f/TEXTURE_PATTERN_DIVIDE_Y );
...
void SetTexturePlayer( int cntPattern )
{
	...
	float sizeY       = 1.0f / TEXTURE_PATTERN_DIVIDE_Y;
		...
		g_vertexWkPlayer[0].tex = D3DXVECTOR2( (float)( x ) * sizeX, (float)( y ) * sizeY );
		g_vertexWkPlayer[1].tex = D3DXVECTOR2( (float)( x ) * sizeX + sizeX, (float)( y ) * sizeY );
		g_vertexWkPlayer[2].tex = D3DXVECTOR2( (float)( x ) * sizeX, (float)( y ) * sizeY + sizeY );
		g_vertexWkPlayer[3].tex = D3DXVECTOR2( (float)( x ) * sizeX + sizeX, (float)( y ) * sizeY + sizeY );
}

// 頂点座標の設定
void SetVertexPlayer(void) {
	g_vertexWkPlayer[0].vtx = D3DXVECTOR3(g_posPlayer.x, g_posPlayer.y, 0.0f);
	g_vertexWkPlayer[1].vtx = D3DXVECTOR3(g_posPlayer.x + TEXTURE_PLAYER00_SIZE_X, g_posPlayer.y, 0.0f);
	g_vertexWkPlayer[2].vtx = D3DXVECTOR3(g_posPlayer.x, g_posPlayer.y + TEXTURE_PLAYER00_SIZE_Y, 0.0f);
	g_vertexWkPlayer[3].vtx = D3DXVECTOR3(g_posPlayer.x + TEXTURE_PLAYER00_SIZE_X, g_posPlayer.y + TEXTURE_PLAYER00_SIZE_Y, 0.0f);
}
